...
use crate::hzcu_assist::HzcuAssist;
...
#[tauri::command]
pub async fn login(
    state: State<'_, Arc<Mutex<HzcuAssist>>>,
    handle: AppHandle,
    username: String,
    password: String,
    auto_login: bool,
    ...
    let mut hzcu_assist = state.lock().await;
    hzcu_assist
    ...
    if auto_login {
        let entry = Entry::new("hzcu-assist", "auto-login").map_err(|err| err.to_string())?;
        entry
            .set_password(&format!("{}\n{}", username, password))
            .map_err(|err| err.to_string())?;
    } else {
        let entry = Entry::new("hzcu-assist", "auto-login");
...
#[tauri::command]
pub async fn check_login(state: State<'_, Arc<Mutex<HzcuAssist>>>) -> Result<bool, String> {
    info!("check_login");
    let hzcu_assist = state.lock().await;
    match hzcu_assist.is_login() {
        true => Ok(true),
...
#[tauri::command]
pub async fn test_connection(state: State<'_, Arc<Mutex<HzcuAssist>>>) -> Result<(), String> {
...
pub async fn sync_todo_once(
    hzcu_assist: State<'_, Arc<Mutex<HzcuAssist>>>,
    handle: AppHandle,
) -> Result<Vec<Value>, String> {
    info!("sync_todo_once");
    let hzcu_assist = hzcu_assist.lock().await.clone();
    let todo_list = hzcu_assist
        .get_todo_list()
        .await
        .map_err(|err| err.to_string())?;
    ...
    menu.append_items(&[
        &MenuItem::with_id(
            &handle,
            "id",
            format!("已登录：{}", hzcu_assist.get_username()),
            false,
            None::<&str>,
        )
        .map_err(|err| err.to_string())?,
...
pub fn export_todo(
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        for todo in todo_list_with_end_time.iter() {
            ...
            let title = todo["title"].as_str().unwrap();
            let url = format!(
                "https://courses.hzcu.edu.cn/course/{}/learning-activity#/{}?view=scores",
...
#[tauri::command]
pub async fn get_courses(state: State<'_, Arc<Mutex<HzcuAssist>>>) -> Result<Vec<Value>, String> {
    info!("get_courses");
    let hzcu_assist = state.lock().await.clone();
    hzcu_assist
        .get_courses()
        .await
        .map_err(|err| err.to_string())
...
pub async fn get_uploads_list(
    ...
    for course in courses.as_array().unwrap() {
        ...
        let hzcu_assist = hzcu_assist.clone();
        let save_path = save_path.clone();
        tasks.push(tokio::task::spawn(async move {
            let mut uploads = Vec::new();
            let activities_uploads = hzcu_assist
                .get_activities_uploads(course_id)
                .await
                .map_err(|err| err.to_string())?;
...
pub async fn start_download_ppts(
    ...
    let mut hzcu_assist_mut = hzcu_assist.lock().await;
    hzcu_assist_mut
        .keep_classroom_alive()
        .await
        .map_err(|err| err.to_string())?;
    drop(hzcu_assist_mut);
...
pub async fn get_range_subs(
    ...
    let mut hzcu_assist_mut = state.lock().await;
    hzcu_assist_mut
        .keep_classroom_alive()
        .await
        .map_err(|err| err.to_string())?;
...
#[tauri::command]
pub async fn get_score(state: State<'_, Arc<Mutex<HzcuAssist>>>) -> Result<Vec<Value>, String> {
    info!("get_score");
    let mut hzcu_assist = state.lock().await;
    let score = hzcu_assist
        .get_score()
        .await
        .map_err(|err| err.to_string())?;
    Ok(score)
...
pub async fn set_config(
    ...
    *current_config = config.clone();
    ...
    // save config to file
    if let Some(proj_dirs) = ProjectDirs::from("", "", "HZCU-learning-assistant") {
