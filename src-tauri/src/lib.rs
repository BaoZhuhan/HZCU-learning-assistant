...
fn setup_logging(level: LevelFilter, to_file: bool) -> Result<(), fern::InitError> {
    ...
    base_config = if to_file {
        base_config
            .chain(std::io::stdout())
            .chain(fern::log_file("hzcu-learning-assistant.log")?)
    } else {
...
pub fn run() {
    let hzcu_assist = Arc::new(Mutex::new(HzcuAssist::new()));
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            #[cfg(target_os = "windows")]
            {
                app.tray_by_id("main")
                    .unwrap()
                    .set_tooltip(Some("HZCU Learning Assistant"))
                    .unwrap();
                app.tray_by_id("main")
                    .unwrap()
