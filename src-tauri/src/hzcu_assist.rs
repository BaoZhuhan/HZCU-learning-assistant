...
impl HzcuAssist {
    ...
    pub fn request<U: IntoUrl + Clone>(&self, method: Method, url: U) -> HzcuRequestBuilder {
        HzcuRequestBuilder::new(self.clone(), method, url, self.proxy_first)
    }

    pub fn get<U: IntoUrl + Clone>(&self, url: U) -> HzcuRequestBuilder {
        info!("GET {}", url.as_str());
        self.request(Method::GET, url)
    }

    pub fn post<U: IntoUrl + Clone>(&self, url: U) -> HzcuRequestBuilder {
    ...
    pub async fn login(&mut self, username: &str, password: &str) -> Result<()> {
        ...
        if res.text().await?.contains("统一身份认证平台") {
            ...
            self.get("https://courses.hzcu.edu.cn/user/courses")
                .send()
                .await?;
    ...
    pub async fn get_courses(&self) -> Result<Vec<Value>> {
        ...
        if json["pages"].as_i64().unwrap() > 1 {
            for page in 2..=json["pages"].as_i64().unwrap() {
                let res = self.get(format!("https://courses.hzcu.edu.cn/api/my-courses?conditions=%7B%22status%22:%5B%22ongoing%22,%22notStarted%22%5D,%22keyword%22:%22%22,%22classify_type%22:%22recently_started%22,%22display_studio_list%22:false%7D&fields=id,name,course_code,department(id,name),grade(id,name),klass(id,name),course_type,cover,small_cover,start_date,end_date,is_started,is_closed,academic_year_id,semester_id,credit,compulsory,second_name,display_name,created_user(id,name),org(is_enterprise_or_organization),org_id,public_scope,audit_status,audit_remark,can_withdraw_course,imported_from,allow_clone,is_instructor,is_team_teaching,is_default_course_cover,instructors(id,name,email,avatar_small_url),course_attributes(teaching_class_name,is_during_publish_period,copy_status,tip,data),user_stick_course_record(id),classroom_schedule&page={}&page_size=100&showScorePassedStatus=false", page))
                    .send()
                    .await?;
    ...
    pub async fn get_activities_uploads(&self, course_id: i64) -> Result<Vec<Value>> {
        ...
        let res = self
            .get(format!(
                "https://courses.hzcu.edu.cn/api/courses/{}/activities",
                course_id
            ))
            .send()
            .await?;
    ...
    pub async fn get_homework_uploads(&self, course_id: i64) -> Result<Vec<Value>> {
        ...
        let mut uploads = Vec::new();
        let res = self.get(format!("https://courses.hzcu.edu.cn/api/courses/{}/homework-activities?conditions=%7B%22itemsSortBy%22:%7B%22predicate%22:%22module%22,%22reverse%22:false%7D%7D&page=1&page_size=20&reloadPage=false", course_id))
            .send()
            .await?;
        let json: Value = res.json().await?;
        ...
        if json["pages"].as_i64().unwrap() > 1 {
            for page in 2..=json["pages"].as_i64().unwrap() {
                let res = self.get(format!("https://courses.hzcu.edu.cn/api/courses/{}/homework-activities?conditions=%7B%22itemsSortBy%22:%7B%22predicate%22:%22module%22,%22reverse%22:false%7D%7D&page={}&page_size=20&reloadPage=false", course_id, page))
                    .send()
                    .await?;
                let json: Value = res.json().await?;
                let homeworks = json["homework_activities"].as_array().unwrap();
    ...
    pub async fn download_file(&self, reference_id: i64, name: &str, path: &str) -> Result<()> {
        let res = self
            .get(format!(
                "https://courses.hzcu.edu.cn/api/uploads/reference/{}/blob",
                reference_id
            ))
            .send()
            .await?;
        ...
        let res = match res.status().is_success() {
            true => res,
            false => {
                let res = self.get(format!("https://courses.hzcu.edu.cn/api/uploads/reference/document/{}/url?preview=true", reference_id))
                    .send()
                    .await?;
                let json: Value = res.json().await?;
    ...
    pub async fn get_academic_year_list(&self) -> Result<Vec<Value>> {
        ...
        let res = self
            .get("https://courses.hzcu.edu.cn/api/my-academic-years?fields=id,name,sort,is_active")
            .send()
            .await?;
        let json: Value = res.json().await?;
    ...
    pub async fn get_semester_list(&self) -> Result<Vec<Value>> {
        ...
        let res = self
            .get("https://courses.hzcu.edu.cn/api/my-semesters?")
            .send()
            .await?;
        let json: Value = res.json().await?;
        Ok(json["semesters"]
    ...
    pub async fn get_todo_list(&self) -> Result<Vec<Value>> {
        ...
        let res = self
            .get("https://courses.hzcu.edu.cn/api/todos?no-intercept=true")
            .send()
            .await?;
        let json: Value = res.json().await?;
        Ok(json["todo_list"]
    ...
    pub fn get_token(&self) -> Result<String> {
        ...
        if let Some(cookies) = self
            .jar
            .cookies(&url::Url::parse("https://classroom.hzcu.edu.cn")?)
        {
    ...
    pub async fn get_month_subs(&self, month: &str) -> Result<Vec<Subject>> {
        ...
        let res = self.get(format!("https://classroom.hzcu.edu.cn/courseapi/v2/course-live/get-my-course-month?month={}", month))
            .headers(headers.clone())
            .send()
            .await?;
        let json: Value = res.json().await?;
    ...
    pub async fn get_range_subs(
        ...
        let mut date = start;
        while date <= end {
            let res = self.get(format!("https://classroom.hzcu.edu.cn/courseapi/v2/course-live/get-my-course-day?day={}", date.format("%Y-%m-%d")))
                .headers(headers.clone())
                .send()
                .await?;
            let json: Value = res.json().await?;
    ...
    pub async fn search_courses(
        ...
        let res = self
            .get("https://classroom.hzcu.edu.cn/userapi/v1/infosimple")
            .headers(headers.clone())
            .send()
            .await?;
        let json: Value = res.json().await?;
        ...
        let res = self.get(format!("https://classroom.hzcu.edu.cn/pptnote/v1/searchlist?tenant_id=112&user_id={}&user_name={}&page=1&per_page=16&title={}&realname={}&trans=&tenant_code=112&randomKey={}", user_id, account, course_name, teacher_name, random))
            .headers(headers.clone())
            .send()
            .await?;
        ...
        while courses.len() < total_course as usize {
            page += 1;
            let random: f64 = rand::random();
            let res = self.get(format!("https://classroom.hzcu.edu.cn/pptnote/v1/searchlist?tenant_id=112&user_id={}&user_name={}&page={}&per_page=16&title={}&realname={}&trans=&tenant_code=112&randomKey={}", user_id, account, page, course_name, teacher_name, random))
                .headers(headers.clone())
                .send()
                .await?;
            let json: Value = res.json().await?;
    ...
    pub async fn get_playback_response(&self, course_id: i64, sub_id: i64) -> Result<Response> {
        let res = self
            .get(format!(
                "https://classroom.hzcu.edu.cn/courseapi/v3/portal-home-setting/get-sub-info?course_id={}&sub_id={}",
                course_id, sub_id
            ))
            .send()
            .await?;
    ...
    pub async fn get_ppt_urls(&self, course_id: i64, sub_id: i64) -> Result<Vec<String>> {
        ...
        let res = self.get(format!("https://classroom.hzcu.edu.cn/pptnote/v1/schedule/search-ppt?course_id={}&sub_id={}&page=1&per_page=100", course_id, sub_id)).send()
            .await?;
        let json: Value = res.json().await?;
        ...
        while urls.len() < total_ppt as usize {
            page += 1;
            let res = self.get(format!("https://classroom.hzcu.edu.cn/pptnote/v1/schedule/search-ppt?course_id={}&sub_id={}&page={}&per_page=100", course_id, sub_id, page)).send()
                .await?;
            let json: Value = res.json().await?;
